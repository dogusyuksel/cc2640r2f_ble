...
// Minimum connection interval (units of 1.25ms, 80=100ms) for parameter update
// request
#define DEFAULT_DESIRED_MIN_CONN_INTERVAL 80

// Maximum connection interval (units of 1.25ms, 104=130ms) for  parameter
// update request
...
// Supervision timeout value (units of 10ms, 300=3s) for parameter update
// request
...
// Delay (in ms) after connection establishment before sending a parameter
// update requst
...
#define SIMPLEPERIPHERAL_ASSERT(expr)                                          \
  if (!(expr))                                                                 \
    simple_peripheral_spin();
...
static void simple_peripheral_sendToNPI(uint8_t *buf,
                                        uint16_t len); // Declaration
#endif                                                 // PTM_MODE
...
static void simple_peripheral_sendToNPI(uint8_t *buf, uint16_t len) {
  ...
  if (pNpiPkt) {
    pNpiPkt->hdr.event =
        buf[0]; // Has the event status code in first byte of payload
...
#define APP_SUGGESTED_TX_TIME 2120 // default is 328us(TX)
    ...
    // See the LE Data Length Extension section in the BLE5-Stack User's Guide
...
static void SimplePeripheral_processGapMessage(gapEventHdr_t *pMsg) {
  ...
  case GAP_DEVICE_INIT_DONE_EVENT: {
    ...
    if (pPkt->hdr.status == SUCCESS) {
      ...
      // Temporary memory for advertising parameters for set #1. These will be
      // copied by the GapAdv module
      ...
      // Load advertising data for set #1 that is statically allocated by the
      // app
      status = GapAdv_loadByHandle(advHandleLegacy, GAP_ADV_DATA_TYPE_ADV,
                                   sizeof(advertData), advertData);
      SIMPLEPERIPHERAL_ASSERT(status == SUCCESS);

      // Load scan response data for set #1 that is statically allocated by the
      // app
      ...
      // Load advertising data for set #2 that is statically allocated by the
      // app
      ...
      if (addrMode > ADDRMODE_RANDOM) {
        ...
        Util_constructClock(&clkRpaRead, SimplePeripheral_clockHandler,
                            SP_READ_RPA_EVT_PERIOD, 0, true, (UArg)&argRpaRead);
      }
#endif // PRIVACY_1_2_CFG
    }
  ...
  case GAP_LINK_PARAM_UPDATE_EVENT: {
    ...
    spConnHandleEntry_t *connHandleEntry =
        (spConnHandleEntry_t *)List_get(&paramUpdateList);
...
bool SimplePeripheral_doSetConnPhy(uint8 index) {
  ...
  static uint8_t phy[] = {HCI_PHY_1_MBPS,
                          HCI_PHY_2_MBPS,
  ...
  // Note PHYs are already enabled by default in build_config.opt in stack
  // project.
...
static uint8_t SimplePeripheral_addConn(uint16_t connHandle) {
  ...
  for (i = 0; i < MAX_NUM_BLE_CONNS; i++) {
    if (connList[i].connHandle == CONNHANDLE_INVALID) {
      ...
      connList[i].pParamUpdateEventData =
          ICall_malloc(sizeof(spClockEventData_t) + sizeof(uint16_t));
...
static void SimplePeripheral_processCmdCompleteEvt(hciEvt_CmdComplete_t *pMsg) {
  ...
  case HCI_READ_RSSI: {
    ...
    // Display RSSI value, if RSSI is higher than threshold, change to faster
    // PHY
    if (status == SUCCESS) {
      uint16_t handle =
      ...
      if (rssi != LL_RSSI_NOT_AVAILABLE) {
        ...
        if (connList[index].phyCngRq == FALSE) {
          ...
          if ((phyRq != SP_PHY_NONE) &&
              // First check if the request for this phy change is already not
              // honored then don't request for change
              (((connList[index].rqPhy == phyRq) &&
                (connList[index].phyRqFailCnt < 2)) ||
               (connList[index].rqPhy != phyRq))) {
            // Initiate PHY change based on RSSI
            SimplePeripheral_setPhy(connList[index].connHandle, 0, phyRqS,
                                    phyRqS, phyOpt);
            connList[index].phyCngRq = TRUE;

            // If it a request for different phy than failed request, reset the
            // count
...
static status_t SimplePeripheral_startAutoPhyChange(uint16_t connHandle) {
  ...
  status = Gap_RegisterConnEventCb(SimplePeripheral_connEvtCB, GAP_CB_REGISTER,
                                   connHandle);
  ...
  case HCI_LE_SET_PHY: {
    ...
    if (connHandleEntry) {
      ...
      if (connIndex < MAX_NUM_BLE_CONNS) {
        ...
        if (pMyMsg->cmdStatus == HCI_ERROR_CODE_UNSUPPORTED_REMOTE_FEATURE) {
          // Update the phychange request status for active RSSI tracking
          // connection
          connList[connIndex].phyCngRq = FALSE;
...
bool SimplePeripheral_doEnablePTMMode(uint8_t index) {
  ...
  NPITask_registerIncomingRXEventAppCB(
      simple_peripheral_handleNPIRxInterceptEvent, INTERCEPT);

  // Register for Command Status information
  HCI_TL_Init(NULL, (HCI_TL_CommandStatusCB_t)simple_peripheral_sendToNPI, NULL,
              selfEntity);

  // Register for Events
  HCI_TL_getCmdResponderID(
